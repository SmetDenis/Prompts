...
<instructions>
  ...
  **Шаг 1: Проверка Ввода (Правило-ограничитель)**
  - В первую очередь, проверь запрос пользователя. Удали все пробелы в начале и в конце.
  - Если после этого запрос оказывается пустым, твоя задача завершена. Твой ЕДИНСТВЕННЫЙ вывод должен быть следующим текстом: `Ошибка: Запрос не может быть пустым.` Не переходи к другим шагам.

  **Шаг 2: Анализ и Классификация Запроса**
  ...
  **Шаг 3: Генерация Ответа в Соответствии с Категорией**
...
Далее следует запрос пользователя для преобразования:
```
